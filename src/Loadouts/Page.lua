...
-- The loadout page
--
-- One named pair of weapons, a stance and a key, drawn as the widget kit draws
-- everything else. It used to be a section of the options window and it is a
-- tab of the character window now, which is a move rather than a rewrite: the
-- kit takes a host, the character window is a host, and the whole of the change
-- is which frame the rows land on.
--
-- The move is the point. A loadout is your gear on a key. Reading it beside a
-- slider for how opaque the chat window is was reading it in the wrong place,
-- and the page you go to when you want to change your weapons is the page with
-- your weapons on it.
--
-- Its own file rather than a function inside Loadouts/Feature.lua, because a
-- Feature.lua is a registration and this is a page somebody else draws. Nothing
-- here knows what a window is; it takes a kit and calls the same nine functions
-- every other page in the addon calls.
--------------------------------------------------------------------------
...
function Page.Build(ui)
	...
	-- The strip under the two hands, the way the client's own sheet puts its
	-- tabs under the paperdoll. Every loadout is a row in it and + adds one.
	ui.Tabs(
		function()
			local labels = {}
